...
{
	ESP8266_RST_HIGH_LEVEL();    //片选，选中ESP8266
	
	Delay_ms ( 1000 ); 
	
	while ( ! ESP8266_Cmd ( "AT", "OK", NULL, 200 ) ) ESP8266_Rst ();  			//不管如何，都要复位
...
{    
	...
	if ( ( reply1 != 0 ) && ( reply2 != 0 ) )         //两个都有回复要求
		return ( ( bool ) strstr ( strEsp8266_Fram_Record .Data_RX_BUF, reply1 ) || 
						 ( bool ) strstr ( strEsp8266_Fram_Record .Data_RX_BUF, reply2 ) ); 
 	
	else if ( reply1 != 0 )								//只有一个
		return ( ( bool ) strstr ( strEsp8266_Fram_Record .Data_RX_BUF, reply1 ) );
...
bool ESP8266_BuildAP ( char * pSSID, char * pPassWord, char * enunPsdMode )
{
	...
	sprintf ( cCmd, "AT+CWSAP=\"%s\",\"%s\",1,%s", pSSID, pPassWord, enunPsdMode );//字符串格式化命令，组成cCMD
...
void ESP8266_STA_TCP_Client ( void )
{
  ...
  do{
		pStrDelimiter [0]="601";
		pStrDelimiter [1]="12345612";
  } while ( ! ESP8266_JoinAP ( pStrDelimiter [0], pStrDelimiter [1] ) );
	ESP8266_Enable_MultipleId ( ENABLE );
	do {
		pStrDelimiter [0]="192.168.191.1";
		pStrDelimiter [1]="8080 ";
